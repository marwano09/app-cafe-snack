...
<div class="max-w-6xl mx-auto">
  ...
  <nav class="mb-4 text-sm text-neutral-500 dark:text-neutral-400" aria-label="Breadcrumb">
    <ol class="flex items-center gap-2">
      ...
      <li class="text-neutral-900 dark:text-neutral-200 font-medium">إضافة صنف</li>
    </ol>
  </nav>
  ...
  <header class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
    <div>
      <h1 class="text-2xl sm:text-3xl font-semibold tracking-tight text-neutral-900 dark:text-neutral-100">إضافة صنف</h1>
      <p class="mt-1 text-sm text-neutral-500 dark:text-neutral-400">أدخل تفاصيل الصنف، ارفع الصورة، واضبط التوفّر.</p>
    </div>
    <a href="{{ $backUrl }}"
  ...
  <form id="createForm" action="{{ route('menu.store') }}" method="post" enctype="multipart/form-data" class="space-y-6">
    @csrf
    ...
    <div class="grid lg:grid-cols-3 gap-6">
      ...
      <section class="lg:col-span-2 space-y-6">
        ...
        <div class="rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-5 shadow-sm">
          <h2 class="text-base font-semibold text-neutral-900 dark:text-neutral-100 mb-4">البيانات الأساسية</h2>
          ...
          <div class="grid md:grid-cols-2 gap-4">
            <div>
              ...
              <div class="text-[11px] text-neutral-500 mt-1">اكتب اسمًا واضحًا وقصيرًا.</div>
            ...
            <div>
              ...
              <input id="price" type="number" step="0.01" inputmode="decimal" name="price" value="{{ old('price') }}" required
                     class="w-full rounded-xl border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-900 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400">
              <div class="text-[11px] text-neutral-500 mt-1">سيُعرض للزبون بالدرهم.</div>
            ...
            <div>
              ...
              <label class="inline-flex items-center gap-3 select-none">
                ...
                <input type="checkbox" name="is_available" value="1"
                       @checked(old('is_available','1')==='1') class="sr-only peer">
                <span class="w-11 h-6 rounded-full bg-neutral-300 dark:bg-neutral-700 relative transition
                             peer-checked:bg-emerald-500 after:content-[''] after:absolute after:top-0.5 after:start-0.5
                ...
                <span class="text-sm text-neutral-700 dark:text-neutral-300" id="availabilityLabel">
                  {{ old('is_available','1')==='1' ? 'متاح' : 'غير متاح' }}
                </span>
              </label>
            ...
            <div class="md:col-span-2">
              ...
              <textarea id="description" name="description" rows="4" maxlength="400"
                        class="w-full rounded-xl border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-900 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400"
                        placeholder="مكوّنات، طريقة تقديم، ملاحظات حساسية...">{{ old('description') }}</textarea>
              @error('description')<div class="text-rose-600 text-xs mt-1">{{ $message }}</div>@enderror
            </div>

            {{-- Optional fields (ready for future use) --}}
            <div>
            ...
            <div>
              ...
              <input id="tags" name="tags" value="{{ old('tags') }}" placeholder="حار, نباتي, خالي من الغلوتين..."
                     class="w-full rounded-xl border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-900 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400">
      ...
      <aside class="space-y-6">
        ...
        <div class="rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-5 shadow-sm">
          ...
          <article class="group rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-4">
            <div class="flex items-start gap-3">
              <div class="relative">
                <img id="liveCardImage" src="{{ asset('images/placeholder.webp') }}" class="w-20 h-20 object-cover rounded-xl ring-1 ring-black/5 dark:ring-white/10" alt="">
                <span id="liveAvail" class="absolute -end-2 -top-2 inline-flex items-center rounded-full text-[10px] px-2 py-0.5 border bg-emerald-50 text-emerald-700 border-emerald-200">
                  {{ old('is_available','1')==='1' ? 'متاح' : 'غير متاح' }}
                </span>
              </div>
              <div class="min-w-0 flex-1">
                <div class="flex items-start justify-between gap-3">
                  <div class="min-w-0">
                    <div id="liveName" class="font-semibold text-base truncate">{{ old('name') ?: '—' }}</div>
                    <div class="text-xs text-neutral-500 dark:text-neutral-400 mt-1 truncate" id="liveCategory">
                      الفئة: {{ old('category_id') ? optional($categories->firstWhere('id', old('category_id')))->name : '—' }}
                    </div>
                  </div>
                  <div id="livePrice" class="text-sm font-bold text-green-700 dark:text-green-400 shrink-0">
                    DH {{ old('price') ? number_format((float)old('price'),2) : number_format(0,2) }}
                  </div>
                </div>
                <p id="liveDesc" class="text-[13px] text-neutral-600 dark:text-neutral-300 mt-2 line-clamp-2">{{ old('description') }}</p>
              </div>
            </div>
          </article>
        ...
        {{-- Tips --}}
        <div class="rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-5 shadow-sm">
          <h3 class="text-sm font-semibold text-neutral-900 dark:text-neutral-100 mb-2">نصائح سريعة</h3>
          <ul class="text-xs text-neutral-600 dark:text-neutral-300 space-y-1 leading-5">
            <li>استخدم أسماء موجزة، ووصفًا لا يتجاوز 2-3 أسطر.</li>
            <li>اختر صورًا مربعة واضحة (800×800 على الأقل) لضمان جودة رائعة.</li>
            <li>استعمل الوسوم لتمييز العناصر (حار، نباتي، بدون جلوتين...).</li>
          </ul>
        </div>
      </aside>
    </div>
      ...
      <div class="rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white/90 dark:bg-neutral-900/90 backdrop-blur p-3 flex items-center justify-between gap-2 shadow-sm">
        ...
        <div class="flex items-center gap-2">
          ...
          <button id="saveBtn"
            ...
            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M12 7v10m5-5H7"/></svg>
            حفظ ⌘/Ctrl+S
          </button>
        </div>
      </div>
...
<script>
  (function () {
    ...
    // Bind inputs to live preview + counters
    ...
    category?.addEventListener('change', () => {
      markDirty();
      const txt = category.options[category.selectedIndex]?.text || '—';
      liveCategory.textContent = 'الفئة: ' + txt;
    });
