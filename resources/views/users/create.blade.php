...
<div class="mb-5 flex items-center justify-between">
  <div>
    <h1 class="text-2xl font-semibold tracking-tight text-neutral-900 dark:text-neutral-100">إنشاء مستخدم</h1>
    <p class="text-sm text-neutral-500 dark:text-neutral-400 mt-1">املأ الحقول المطلوبة، ثم اضغط حفظ.</p>
  </div>
...
<div class="rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-5 shadow-sm">
  <form method="POST" action="{{ route('users.store') }}" class="grid gap-5 max-w-3xl" id="userCreateForm" novalidate>
    @csrf
    ...
    <div class="grid md:grid-cols-2 gap-4">
      ...
      {{-- Username --}}
      <div>
        <label class="block text-sm mb-1 font-medium">اسم المستخدم (اختياري)</label>
        <input name="username" value="{{ old('username') }}" autocomplete="username"
               class="w-full rounded-xl border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-900 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400">
        <div class="text-[11px] text-neutral-500 dark:text-neutral-400 mt-1">يفضّل أحرفًا لاتينية/أرقام.</div>
    ...
    <div class="grid md:grid-cols-2 gap-4">
      <div>
        <label class="block text-sm mb-1 font-medium">البريد الإلكتروني (اختياري)</label>
        <input type="email" name="email" value="{{ old('email') }}" autocomplete="email"
               class="w-full rounded-xl border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-900 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400" dir="ltr">
        <div class="text-[11px] text-neutral-500 dark:text-neutral-400 mt-1">نستخدمه لاستعادة الحساب والإشعارات.</div>
    ...
    {{-- Password --}}
    <div>
      <label class="block text-sm mb-1 font-medium">كلمة المرور <span class="text-rose-600">*</span></label>
      <div class="flex gap-2">
        <div class="relative flex-1">
          <input type="password" name="password" id="passwordInput" required autocomplete="new-password"
                 class="w-full rounded-xl border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-900 px-4 py-2 text-sm pr-10 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400">
...
@push('scripts')
<script>
  // Show/Hide password (no backend impact)
  (function () {
  ...
  (function () {
    ...
    function render(sc) {
      ...
      if (sc >= 4) { label = 'جيدة'; color = '#f59e0b'; }     // amber
      if (sc >= 5) { label = 'قوية'; color = '#10b981'; }     // emerald
